...
QString
rulesToString(const QMap<QByteArray, QString> &rules)
{
	QString s;
	for(auto it = rules.cbegin(); it != rules.cend(); ++it)
		s += QString::fromLatin1(it.key()) + QChar(':') + it.value() + QChar(';');
	return s;
}
...
RichCssTest::testParseRules()
{
	...
	QCOMPARE(rulesToString(RichCSS::parseCssRules(cssIn)), cssOut);
}
